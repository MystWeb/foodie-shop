...
public class DateUtil {
     ...
     * 线程安全的日期格式化，默认生成当前时间
     *
     * @param temporalAccessor TemporalAccessor是框架级接口，定义对时态对象(如日期、时间、偏移量或它们的某些组合)的只读访问。这是日期、时间和偏移量对象的基本接口类型。
     * @param outputFormat     输出格式 例：yyyy/MM/dd HH:mm:ss
     * @return 格式化日期
     */
    public static String formattedDate(TemporalAccessor temporalAccessor, String outputFormat) {
        String defaultFormat = "yyyy/MM/dd HH:mm:ss";

        // 默认生成当前时间，格式为：1900/01/01 00:00:00
        if (Objects.isNull(temporalAccessor) && Objects.isNull(outputFormat)) {
            return DateTimeFormatter.ofPattern(defaultFormat).format(LocalDateTime.now());
        }

        // 如果未设置输出日期格式，则根据日期对象转换对应的默认日期格式
